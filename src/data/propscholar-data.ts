...
const HELP_ARTICLES = [
  ...
  // Add all other HELP_ARTICLES exactly as you provided
];
...
export async function getPropScholarData(): Promise<string> {
  ...
  prompt += `3. If the question is unrelated, reply: "I can only assist with PropScholar-related queries. Please contact support."\n`;
  prompt += `4. DO NOT add unnecessary info.\n`;
  prompt += `5. NEVER guess; if unknown, say: "I don't have that information. Visit help.propscholar.com or contact support."\n`;
  prompt += `6. NO emojis or casual language.\n\n`;
  ...
  Object.entries(categorizedArticles).forEach(([category, articles]) => {
    ...
    articles.forEach(article => {
      ...
      prompt += `URL: ${cleanUrl(article.url)}\n`;
      prompt += `Content: ${article.content}\n\n`;
    });
  });

  prompt += `## CITATION FORMAT:\n- Source URLs are maintained internally but NOT shown in bot replies.\n\n`;
  prompt += `Remember: Short, professional answers without URLs or emojis.`;

  return prompt;
}

// Clean any trailing symbols from AI responses
