...
class MoodService {
  ...
  professionalTone(mood: string): string {
    switch (mood) {
      ...
      case "confused": return "Explain step-by-step.";
      default: return "Use a clean professional tone.";
    }
  }
}
...
import BrainService from "./services/brain.service";   // <<< IMPORTANT ADDITION
...
/* -------------------------------------------------------
   UPGRADED MESSAGE HANDLER (ADMIN BRAIN)
------------------------------------------------------- */
...
client.on("messageCreate", async (msg) => {
  ...
  if (botTagged && isModerator) {
    ...
    if (replyMatch) {
      ...
      await msg.reply("✅ Replied to user!");
      return;
    }
  ...
  /* -------------------------------------------------------
      MODERATOR WAIT SYSTEM (20 seconds)
  ------------------------------------------------------- */
  ...
  if (modReplied) {
    console.log("⏭️ Moderator handled it. Skipping bot.");
    return;
  }
